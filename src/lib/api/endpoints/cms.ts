...
export const cmsApi = {
  ...
  list(params?: { page?: number; limit?: number; sort?: string; tags?: string[] }) {
    ...
    return api.get<PaginatedResponse<Post>>(`${base}/posts${query ? `?${query}` : ''}`)
  },
  ...
  search(q: string, params?: { page?: number; limit?: number }) {
    ...
    if (params?.page) qs.set('page', String(params.page))
    if (params?.limit) qs.set('limit', String(params.limit))
    return api.get<PaginatedResponse<Post>>(`${base}/posts/search?${qs.toString()}`)
  },
}
