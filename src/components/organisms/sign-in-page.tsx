...
import { BookOpen, Github, Globe, Loader2, Mail, ArrowLeft } from 'lucide-react'
...
function OAuthButtons({
  ...
  return (
    <div className="space-y-3">
      <Button
        className="w-full gap-3" variant="outline" size="lg"
        disabled={!!loading} onClick={() => onOAuth('GOOGLE')}
      >
        {loading === 'GOOGLE' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Globe className="h-4 w-4" />}
        {loading === 'GOOGLE' ? 'Connexion...' : 'Continuer avec Google'}
      </Button>

      <Button
        className="w-full gap-3" variant="outline" size="lg"
        disabled={!!loading} onClick={() => onOAuth('GITHUB')}
      >
      ...
      <Button
        className="w-full gap-3" variant="outline" size="lg"
        disabled={!!loading} onClick={() => onOAuth('MICROSOFT')}
      >
      ...
      <Button
        className="w-full gap-3" variant="outline" size="lg"
        disabled={!!loading} onClick={onSwitchToOtp}
      >
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button type="submit" className="w-full" disabled={loading}>
        {loading ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Envoi...</> : 'Envoyer le code'}
      </Button>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        <p className="text-sm text-muted-foreground">
          Code envoyé à <strong className="text-foreground">{email}</strong>
        </p>
...
export function SignInPage() {
  ...
  const exchangeCode = useCallback(async (provider: OAuthProvider, code: string) => {
    const res = await createToken({ code, provider })
    const target = window.opener ? window : null

    if (res?.data?.success) {
      target?.postMessage?.({ type: 'OAUTH_SUCCESS' }, window.location.origin)
      if (window.opener) window.close()
      return
    }

    const message = res?.validationErrors?._errors?.join(', ') ?? 'Une erreur est survenue.'
    target?.postMessage?.({ type: 'OAUTH_ERROR', message }, window.location.origin)
    if (window.opener) window.close()
  }, [])
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              {tab === 'oauth' && 'Sans mot de passe — via OAuth ou code email'}
              {tab === 'otp-email' && 'Entrez votre adresse email'}
              {tab === 'otp-code' && 'Entrez votre code de connexion'}
            </p>
          </div>
