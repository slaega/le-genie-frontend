...
export function AdminUsersTable({ initialData, currentPage, initialSearch }: Props) {
  ...
  async function toggleRole(user: AdminUser) {
    ...
    try {
      ...
      setData((prev) => ({
        ...prev,
        items: prev.items.map((u) => (u.id === user.id ? { ...u, ...updated } : u)),
      }))
  ...
  async function toggleSuspend(user: AdminUser) {
    try {
      const updated = await adminApi.updateUser(user.id, { suspended: !user.suspended })
      setData((prev) => ({
        ...prev,
        items: prev.items.map((u) => (u.id === user.id ? { ...u, ...updated } : u)),
      }))
      toast.success(user.suspended ? 'Utilisateur réactivé' : 'Utilisateur suspendu')
    } catch {
              ...
              <tr key={user.id} className="hover:bg-gray-800/50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <Badge
                    ...
                    className={user.role === 'ADMIN' ? 'bg-indigo-600 hover:bg-indigo-600' : ''}
                  >
                    {user.role}
                  </Badge>
                </td>
                <td className="px-4 py-3">
                  <Badge variant={user.suspended ? 'destructive' : 'outline'} className="text-xs">
                  ...
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      className={`h-7 w-7 hover:bg-gray-700 ${
                        user.suspended ? 'text-green-400 hover:text-green-300' : 'text-red-400 hover:text-red-300'
                      }`}
