...
function parseContent(raw: Record<string, unknown> | string | null | undefined) {
...
export function BlogEditor({
  ...
  const scheduleAutoSave = useCallback(
    (editor: Editor) => {
      ...
      saveTimer.current = setTimeout(async () => {
        setIsSaving(true)
        try {
          await onAutoSave(editor.getJSON() as Record<string, unknown>)
  ...
  return (
    <div className={cn('flex flex-col border rounded-xl overflow-hidden bg-background shadow-sm', className)}>
      <EditorToolbar editor={editor} onImageUpload={onImageUpload} />
      <EditorBubbleMenu editor={editor} />
      <div className="flex-1 overflow-y-auto">
        <EditorContent editor={editor} />
      </div>
      <EditorStatsBar editor={editor} isSaving={isSaving} lastSaved={lastSaved} />
    </div>
  )
}
