...
export default async function AdminDashboard() {
  ...
  const globalCards = [
    ...
    { label: 'Publications', value: stats.totalPosts, icon: FileText, color: 'text-green-400' },
    { label: 'Commentaires', value: stats.totalComments, icon: MessageSquare, color: 'text-yellow-400' },
    { label: 'Abonnés newsletter', value: stats.totalSubscribers, icon: Mail, color: 'text-purple-400' },
    { label: 'Abonnements (follow)', value: stats.totalFollows, icon: Heart, color: 'text-rose-400' },
  ]

  const statusCards = [
    { label: 'Publiés', value: stats.postsByStatus.PUBLISHED, icon: BookOpen, color: 'text-green-400' },
    { label: 'Brouillons', value: stats.postsByStatus.DRAFT, icon: PenLine, color: 'text-yellow-400' },
    { label: 'Archivés', value: stats.postsByStatus.ARCHIVED, icon: Archive, color: 'text-gray-400' },
    { label: 'Vides', value: stats.postsByStatus.EMPTY, icon: FileText, color: 'text-gray-600' },
  ]
    ...
    <div className="space-y-10">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { href: '/admin/users', label: 'Gérer les utilisateurs', desc: 'Rôles, suspensions', icon: Users },
            { href: '/admin/posts', label: 'Gérer les articles', desc: 'Archiver, supprimer', icon: FileText },
            { href: '/admin/subscribers', label: 'Gérer les abonnés', desc: 'Newsletter', icon: Mail },
