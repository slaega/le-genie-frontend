...
export default async function AdminSubscribersPage({
  ...
  try {
    data = await serverApi.get<PaginatedResponse<AdminSubscriber>>(`admin/subscribers?${qs}`)
  } catch {
    // affiche vide
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">
          {data.total.toLocaleString('fr-FR')} abonné{data.total !== 1 ? 's' : ''} inscrits
        </p>
      </div>
      <AdminSubscribersTable initialData={data} currentPage={Number(page)} />
    </div>
  )
}
